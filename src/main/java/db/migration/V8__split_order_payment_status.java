...
public class V8__split_order_payment_status extends BaseJavaMigration {

  private static final String STATUS_CHECK =
      "status IN ('CREATED','PREPARING','SHIPPED','DELIVERED','CANCELED')";
  ...
  public void migrate(Context context) throws Exception {
    ...
    try (Statement statement = connection.createStatement()) {
      statement.execute("ALTER TABLE customer_orders ADD COLUMN IF NOT EXISTS payment_status VARCHAR(30) NOT NULL DEFAULT 'PENDING'");
      statement.execute("UPDATE customer_orders SET payment_status = 'PAID', status = 'CREATED' WHERE status = 'PAID'");
      statement.execute("UPDATE customer_orders SET payment_status = 'PENDING', status = 'CREATED' WHERE status = 'PENDING_PAYMENT'");
      statement.execute("UPDATE customer_orders SET payment_status = 'CANCELED' WHERE status = 'CANCELED' AND payment_status = 'PENDING'");
